...
maxcount= 100
nb_of_captors= 9
port = Serial('COM4', 115200)  #UNO (6 pins)
port2= Serial('COM7', 115200)  #micro  (3 pins)
nb_dif_colors= 20
...
mypen.penup()
for i in range(9):
    mypen.setposition(startmypen[0]+ pas/2 + i%3 * pas,startmypen[1]+ pas/3 + i//3 * pas)
...
captors_size= 12
captors_startpos= (400-(captors_size*8 * 3),-320)
...
############## Functions
    ...
    def color_turtle(nb,turtle):
        ...
        if nb == None:
            ...
            turtle.color(colors[(nb * ((len(colors)-1))//1024)])
    ...
    while count < maxcount:
        ...
        for i in range(6):
            ...
            l[i][count]= voltage*5000/1024
            dist[i]=voltage
        for j in range(3):
            i=j+6
            voltage= get_reading(port2)
            color_turtle(voltage,t[i])
            l[i][count]= voltage*5000/1024
            dist[i]=voltage
        print("dist= ",dist)
